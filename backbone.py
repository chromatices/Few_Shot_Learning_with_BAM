...
from layers import init_layer, distLinear, Flatten, Conv2d_fw, BatchNorm2d_fw
...
class ConvNet(nn.Module):
    ...
    def __init__(self, depth, flatten=True):
        ...
        # Create BAM when option BAM is true
        if self.bam:
            self.BAM = BAM(64)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, list_of_num_layers, list_of_out_dims, flatten=True):
        ...
        trunk = [conv1, bn1, relu, pool1] # like self.conv1 at BAM resnet
        ...
        for i in range(4):
            for j in range(list_of_num_layers[i]):
                ...
                if self.bam:
                    # addiction BAM where Conv Block Layer is end
                    if j == list_of_num_layers[i] -1 and i != 3:
                        self.BAM = BAM(64 * indim // 64)
                        trunk.append(self.BAM)
        if flatten:
...
def ResNet152(flatten=True):
    return ResNet(BottleneckBlock, [3, 8, 36, 3], [256, 512, 1024, 2048], flatten)
